		...
		<div class="card">
			<div class="card-body p-5">
				<div class="border p-4 rounded">
					<form method="post" action="{!! route('SpecificationTypes Store') !!}" enctype="multipart/form-data">
						@csrf
						...
						<div class="row">
							<div class="col-sm-12">
								...
								<a href="{!! route('SpecificationTypes') !!}" class="btn btn-outline-danger px-3">Cancel</a>
							</div>
						</div>
					</form>
				</div>
...
<script type="text/javascript">
	$(document).ready(function(){
		...
		$(add_button).click(function(e){
			e.preventDefault();
			if(count < max_fields){ 
				$(wrapper).append('<div class="row mb-3 itemDivRow"><div class="col-sm-8"><input type="text" name="title[]" value="" id="title" class="form-control" placeholder="Title" required="required"></div><div class="col-sm-3"><select name="status[]" class="form-select" id="status" required="required"><option  value="1">Active</option><option value="2">Inactive</option></select></div><div class="col-sm-1"><button type="button" class="btn btn-outline-danger removeItem"><i class="bx bx-trash"></i></button></div></div>');
			}
		});
