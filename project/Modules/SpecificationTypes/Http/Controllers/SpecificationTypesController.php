...
class SpecificationTypesController extends Controller
{
  public function __construct()
  {
    ...
    public function store(Request $request)
    {
      ...
      try{
       ...
       if(!empty($request->title) && (count($request->title)>0)){
        foreach($request->title as $key => $title){
         ...
         $dataInfo->slug = implode('_', explode(' ', strtolower(strip_tags($title))));
     ...
     $ref = !empty($request->ref) ? $request->ref : (isset($request->exit) ? route('SpecificationTypes') : route('SpecificationTypes Create'));
     return redirect()->to($ref)->with('successMessage', 'Specification Type has been created successfully!.');
